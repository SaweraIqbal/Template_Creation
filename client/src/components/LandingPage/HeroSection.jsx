import { Link } from 'react-router-dom';
import video1 from '../../assets/video1.mp4';
import video2 from '../../assets/video2.mp4';


const HeroSection = () => {
  return (
    <div className="flex flex-col items-center mt-6 lg:mt-12">
        <h1 className="text-4xl sm:text-txl lg:text-7xl text-center tracking-wide">
        Built your <span className='bg-gradient-to-r from-orange-500 to-red-800 text-transparent bg-clip-text'>
        Dream Website
        </span> designs with ease 
        </h1>
        <p className='mt-10 text-lg text-center text-neutral-500  max-w-4xl'>
        TemplaTrove simplifies web design for all skill levels with its intuitive drag-and-drop interface, real-time previews, and customizable elements, enabling users to create stunning, professional websites that match their unique vision.
        </p>
        <div className="flex justify-center my-10">
            <a href='/dashboard/home' className='bg-gradient-to-r from-orange-500 to-orange-800 py-3 px-4 mx-3 rounded-md '>Try It Now</a>
            <a href='#' className='py-3 px-4 mx-3 rounded-md border'>
                Documentation
            </a>
        </div>
        <div className='flex mt-10 justify-center'>
<video autoPlay loop muted className='rounded-lg w-1/2 border border-orange-700 shadow-orange-400 mx-2 my-4'>
<source src={video1} type='video/mp4'/>Your Browser does not show the video tag.</video>
<video autoPlay loop muted className='rounded-lg w-1/2 border border-orange-700 shadow-orange-400 mx-2 my-4'>
<source src={video2} type='video/mp4'/>Your Browser does not show the video tag.</video>
        </div>
    </div>
  )
}

export default HeroSection
